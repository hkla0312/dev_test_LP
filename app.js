...
function artistProfile(artist) {
  ...
  const social = [["Xアカウント", "x", artist.xUrl], ["YouTube", "youtube", artist.youtubeUrl], ["Music", "music", artist.musicUrl]].filter(([, , url]) => isUrl(url));
...
function drawSignalChart(canvas) {
  const context = canvas.getContext("2d"), values = canvas.dataset.values.split(",").map(Number), total = values.reduce((sum, value) => sum + value, 0) || 1, center = 160, radius = 76;
  ...
  context.clearRect(0, 0, 320, 320); context.strokeStyle = "#e8e6ee"; context.lineWidth = 1;
  ...
  SIGNAL_AXIS_META.forEach(([, label, color], index) => { const [x, y] = point(index); context.beginPath(); context.moveTo(center, center); context.lineTo(x, y); context.strokeStyle = color; context.globalAlpha = .4; context.stroke(); context.globalAlpha = 1; const [lx, ly] = point(index, 1.3); context.fillStyle = color; context.font = "600 13px 'Noto Sans JP', sans-serif"; context.textAlign = lx < center - 4 ? "left" : lx > center + 4 ? "right" : "center"; context.fillText(label, lx, ly + 4); });
