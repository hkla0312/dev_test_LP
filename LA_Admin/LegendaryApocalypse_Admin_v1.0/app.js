...
(() => {
  ...
  function onboxPage() {
    ...
    return pageHead('ONBOX 配信設定', '配信当日の設定・進行・コメントモデレーションを管理します。', '<span class="onbox-ui-only">UI PREVIEW</span>') +
      ...
      `<section class="card"><div class="onbox-section-head"><div><h2>5. コメントのリアルタイムモデレーション</h2><p class="sub">受信コメントを確認し、表示・非表示を判断する領域です。</p></div><span class="tag">UIモック</span></div><div class="table-wrap"><table><thead><tr><th>投稿者</th><th>コメント</th><th>状態</th><th>操作</th></tr></thead><tbody><tr><td>DANCHO</td><td>音が少し小さいかもです</td><td><span class="tag">配信確認</span></td><td><button class="secondary" type="button" data-onbox-comment>表示</button><button class="danger" type="button" data-onbox-comment>非表示</button></td></tr><tr><td>MEM-7K2P</td><td>最高！</td><td><span class="tag">表示中</span></td><td><button class="secondary" type="button" data-onbox-comment>表示</button><button class="danger" type="button" data-onbox-comment>非表示</button></td></tr></tbody></table></div></section>`;
  }
  ...
  function signalPage() {
    ...
    const signals = artist ? state.data.artistSignals.filter(signal => signal.artistId === artist.id && (state.showDeletedSignals || !signal.isDeleted)) : [];
    const body = !artist ? '<p class="empty">アーティストを選択してください。</p>' : table(signals, ['MEMBER ID','表示名','種別','コメント','送信日時','操作'], signal => `<tr><td>${escapeHtml(signal.memberId)}</td><td>${escapeHtml(signal.memberDisplayName)}</td><td>${escapeHtml(signal.signalType)}</td><td>${escapeHtml(signal.comment)}</td><td>${timestampText(signal.createdAt)}</td><td>${signal.isDeleted ? `<button class="secondary" data-signal-restore="${signal.id}">復元</button>` : `<button class="danger" data-signal-delete="${signal.id}">削除</button>`}</td></tr>`, 'SIGNALはありません。');
    return pageHead('SIGNAL', 'アーティストごとのSIGNALを確認します。') + `<section class="card"><label class="field">アーティスト<select id="signalArtist"><option value="">選択してください</option>${state.data.artists.map(item => `<option value="${item.id}" ${artist && item.id === artist.id ? 'selected' : ''}>${escapeHtml(item.artistKey)} / ${escapeHtml(item.name)}</option>`).join('')}</select></label><label><input type="checkbox" id="signalDeletedToggle" ${state.showDeletedSignals ? 'checked' : ''}> 削除済みを表示</label></section><section class="card">${body}</section>`;
  }
  function logPage() {
